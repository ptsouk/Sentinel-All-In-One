...
Write-Output "Connected to Azure with subscription: " $context.Subscription.Name
$serverUrl = "https://management.azure.com"
$token = ConvertFrom-SecureString -SecureString (Get-AzAccessToken -ResourceUrl $serverUrl -AsSecureString -WarningAction SilentlyContinue).Token -AsPlainText
$authHeader = @{
